import  torch
from    torch import nn
from    torch.nn import functional as F
from    method.gcn.utils import sparse_dropout, dot


class GraphConvolution(nn.Module):


    def __init__(self, input_dim, output_dim, num_features_nonzero,
                 dropout=0.,
                 is_sparse_inputs=False,
                 bias=False,
                 activation = F.relu,
                 featureless=False):
        super(GraphConvolution, self).__init__()


        self.dropout = dropout
        self.bias = bias
        self.activation = activation
        self.is_sparse_inputs = is_sparse_inputs
        self.featureless = featureless
        self.num_features_nonzero = num_features_nonzero
        self.output_dim=output_dim
        self.weight = nn.Parameter(torch.randn(input_dim, output_dim))
        self.bias = None
        if bias:
            self.bias = nn.Parameter(torch.zeros(output_dim))


    def forward(self, inputs):
        if len(inputs[0].size())==3:
            # print('inputs:', inputs)
            features, graphs = inputs
            timesteps=inputs[0].shape[0]
            outs=[]
            emb_gcn = torch.empty(1, features.shape[1], self.output_dim)  #
            emb_gcn = emb_gcn.to('cuda')
            for i in range(timesteps):

                x = features[i, :, :]
                support = graphs[i, :, :]
                #x, support = input
                x = x.float()
                support = support.float()

                if self.training and self.is_sparse_inputs and 0:
                    x = sparse_dropout(x, self.dropout, self.num_features_nonzero)
                elif self.training:
                    x = F.dropout(x, self.dropout)

                # convolve
                if not self.featureless:  # if it has features x
                    if self.is_sparse_inputs:
                        xw = torch.sparse.mm(x, self.weight)
                    else:
                        xw = torch.mm(x, self.weight)
                else:
                    xw = self.weight

                out = torch.sparse.mm(support, xw)

                if self.bias is not None:
                    out += self.bias
                #outs.append(self.activation(out))
                emb_gcn_l1 = self.activation(out)
                emb_gcn_l1 = torch.unsqueeze(emb_gcn_l1, 0)
                emb_gcn = torch.cat([emb_gcn, emb_gcn_l1], dim=0)
            emb_gcn = emb_gcn[torch.arange(emb_gcn.size(0)) != 0]

            return emb_gcn

        else:
            # print('inputs:', inputs)
            x, support = inputs
            x = x.float()
            support = support.float()

            if self.training and self.is_sparse_inputs  and 0 :
                x = sparse_dropout(x, self.dropout, self.num_features_nonzero)
            elif self.training:
                x = F.dropout(x, self.dropout)

            # convolve
            if not self.featureless: # if it has features x
                if self.is_sparse_inputs:
                    xw = torch.sparse.mm(x, self.weight)
                else:
                    xw = torch.mm(x, self.weight)
            else:
                xw = self.weight


            out = torch.sparse.mm(support, xw)

            if self.bias is not None:
                out += self.bias

            return self.activation(out), support

        return
