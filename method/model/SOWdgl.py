...
def build_SOWdgl(cfg, z_dim, device='cuda'):
    ### Load parameters for SOWdgl
    # General
    ...
    # Build model
    model =SOWdgl(cfg=cfg, input_dim=input_dim, x_dim=x_dim, num_class=num_class, z_dim=z_dim, activation=activation,
                  dense_x_h=dense_x_h,
...
class SOWdgl(nn.Module):

    def __init__(self, cfg, input_dim, x_dim, num_class, z_dim=16, activation='tanh',
    ...
    def build(self):
        ...
        self.gcn = GCN(self.input_dim, self.x_dim, num_features_nonzero=False)
        #######################
        #### Deterministic ####
        #######################
        # 1. x_tm1 -> h_t
    ...
    def get_loss(self,y_true, labels, x, y, graphs_ori, z, z_mean, z_logvar, z_mean_p, z_logvar_p, seq_len, batch_size,
        ...
        # loss_recon_adj = torch.sum(adj/adj_p - torch.log(adj/adj_p) -1)
        ...
        if filter_unseen:
            if threshold is None:
                threshold = (probs[y_ori != self.unseen_label_index].mean() + probs[
                    y_ori == self.unseen_label_index].mean()) / 2.0
